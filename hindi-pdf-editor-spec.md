...
- [ ] Open the resulting PDF in at least two different viewers (e.g. Google Drive's built-in viewer and Adobe Acrobat Reader). **Not done** — no Android emulator or physical device was available in the environment where the above code was written and bundle-verified (`tsc`, `eslint`, `prettier`, and `expo export` all pass, confirming the code compiles and bundles, not that it behaves correctly at runtime). Someone with a real device must run this.
- [ ] **Record the result in writing**: do conjuncts render as single joined glyphs (pass) or disconnected pieces (fail — stop and re-evaluate)? Is the text selectable, or does it behave like a flattened image? (Either is an acceptable *pass* for this app's stated requirements — selectability is a bonus, not a requirement — but record which one you got.) **Blocked on the item above.**
